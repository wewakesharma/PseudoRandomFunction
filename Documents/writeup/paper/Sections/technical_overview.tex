...
Note: Since each trinary number can be either 0,1 or 2, we can add up to 255 numbers and never exceed 511. Moreover, if we take a random sample, we can add 256 numbers and the probability of carryover is negligible. We use this to multiply a packed trinary vector with a trinary matrix with 256 columns (see algorithm below).
...
Phase 2 of the protocols(DIDO, oPRF) outputs 256 bits $\in \mathbb{Z}_2$. Phase 3 takes output of phase 2 and performs a matrix vector multiplication with a public randomization matrix $Rmat \in \mathbb{Z}_{3}^{t \times m} $ to output $\mathbb{Z}_{3}^{t} $\\
...
The prime advantage of replacing matrix vector multiplication with lookup table is speed improvement. This is due to creation of lookup table at the preprocessing stage of the protocol. This means the lookup table can be created beforehand and 
...
\begin{itemize}
	...
	\item{Find a perfect value that can balance between less storage and speedy lookup.}
\end{itemize}
...
%TODO: ADD computation and communication results
