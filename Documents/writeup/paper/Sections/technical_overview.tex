...
\begin{tabular}{|c|c|}
    ...
    \hline
    \multirow{3}{*}{Addition} & $\vec{T} = (\vec{l_1} \lor \vec{m_2}) \oplus (\vec{l_2} \lor \vec{m_1})$;\\
    & $MSB_{res} = ( \vec{l_1} \lor \vec{l_2} ) \oplus  \vec{T} $; \\
    &$LSB_{res} = (\vec{m_1} \lor \vec{m_2} ) \oplus \vec{T} $; \\
    \hline
    \multirow{3}{*}{Subtraction} & $\vec{T} = (\vec{l_1} \lor \vec{m_2}) \oplus (\vec{l_2} \lor \vec{m_1})$;\\
    & $MSB_{res} = ( \vec{l_1} \lor \vec{l_2} ) \oplus  \vec{T} $; \\
    &$LSB_{res} = (\vec{m_1} \lor \vec{m_2} ) \oplus \vec{T} $; \\
    \hline
\multirow{3}{*}{Multiplication} & $\vec{T} = (\vec{l_1} \lor \vec{m_2}) \oplus (\vec{l_2} \lor \vec{m_1})$;\\
    & $MSB_{res} = ( \vec{l_1} \lor \vec{l_2} ) \oplus  \vec{T} $; \\
    &$LSB_{res} = (\vec{m_1} \lor \vec{m_2} ) \oplus \vec{T} $; \\
    \hline
\multirow{3}{*}{MUX} & $\vec{T} = (\vec{l_1} \lor \vec{m_2}) \oplus (\vec{l_2} \lor \vec{m_1})$;\\
    & $MSB_{res} = ( \vec{l_1} \lor \vec{l_2} ) \oplus  \vec{T} $; \\
    &$LSB_{res} = (\vec{m_1} \lor \vec{m_2} ) \oplus \vec{T} $; \\
    \hline
