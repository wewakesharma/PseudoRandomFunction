...
int send_socket;//socket handle to send
int recv_socket;//socket handle to receive
...
void topelitz_Party1(PackedZ2<N_ROWS>& b, const std::vector<uint64_t>& A,
    ...
    //Initialize it as party 1(Server)
    init_party1_communication(recv_socket);

    //Call the pi_recv to recv the value of mx
    pi_recv(recv_socket);
...
void topelitz_Party2_1(PackedZ2<N_COLS>& x, int index) {
    ...
    init_party2_communication(send_socket);
