...
void SC_Party2_2( PackedZ2<N_SIZE>& y2, PackedZ3<N_SIZE>& out, int index)
{
    ...
    out.makeFromBits(outp.first,outp.second); //most = first, least = second
}
...
void SC_Party1(PackedZ2<N_SIZE>& y1, PackedZ3<N_SIZE>& r, int index){
    ...
    PackedZ3<N_SIZE> y3, noty3;

    const static PackedZ2<N_SIZE> zVec; //by default constructor initializes to 0
    y3.makeFromBits(zVec,y1);
    noty3.makeFromBits(zVec,~y1); //one minus y1

    PackedZ3<N_SIZE> r0 = r.add(y3); //mod 3
    PackedZ3<N_SIZE> r1 = r.add(noty3); // mod 3

    PackedPairZ2 r0p = std::make_pair(r0.msbs,r0.lsbs);
    PackedPairZ2 r1p = std::make_pair(r1.msbs,r1.lsbs);

    OT_Party1(r0p,r1p,ra,rb,index);
