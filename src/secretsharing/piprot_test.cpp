...
int main()
{
    ...
    uint64_t rxGlobal[4], zGlobal, xGlobal[4];
...
/*
    ...
    //Receiver Part 1-generate bits X and Rx
    ...
    **cout<<endl<<"Printing Wm and Wl"<<endl;
    for(int i = 0; i < 1; i++)
    {
        for(int word_count = 0; word_count < 64; word_count++)
        {
            cout<<endl<<((Wm[i]>>word_count) & 1)<<"\t"<<(Wl[i]>>word_count & 1)<<endl;
            cout<<endl<<(((Wm[i]>>word_count) & 1) << 1)<<endl;
            WGlobal[i] = (((Wm[i]>>word_count) & 1) << 1) | (Wl[i]>>word_count & 1);
        }
        cout<<WGlobal[i]<<endl;
    }*/
