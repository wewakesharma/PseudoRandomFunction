...
std::vector<unsigned int> pi_recv(int& port)//p1 acts as a server
{
    ...
    int serverSd = openSocket(bindStatus,port);
    assert(bindStatus>=0);
    ...
    //Once the socket is open, using the same socket try to send the same msg to p2.
