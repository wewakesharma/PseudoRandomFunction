...
uint64_t MaGlobal[4][256];  //stores Ma value computed by party 1
uint64_t MbGlobal[4];       //computed by party 1
uint64_t mxbitGlobal;
uint64_t MxGlobal[4];   //stores the value of Mx sent by party 2
...
void getMaMb(uint64_t Ma[4][256], uint64_t Mb[4])
{
    ...
    //reset the global variables--Not required, since they new values will be overwritten
    /*
    for (int iRow=0; iRow<4; iRow++)
    {
        for (int jCol = 0; jCol < 256; jCol++)
...
>>>>>>> cd6f466ade8684602c5c08e4f46c24663767b4c8
{
    uint64_t Mx[4];
    uint64_t Ma[4][256];
    uint64_t Mb_first[4];//Mb_first = Ra*Mx
    uint64_t Mb[4];
    //wait to get mx
    getMx(Mx);
...
=======
    ...
    {
        Mb[iCol]= Mb_first[iCol] + B[iCol] - Rb[iCol]; //Mb = Mb_first + B - Rb
    }
>>>>>>> cd6f466ade8684602c5c08e4f46c24663767b4c8
...
{
    uint64_t A_X[4];//intermediate polynomial evaluation, the variable term
    wordPackedVecMatMult(A,X,A_X);
    for(int cnt = 0; cnt < 4; cnt++)
    {
        global_res[cnt] = A_X[cnt] + B[cnt];
        //cout<<global_res[cnt]<<endl;
    }
}
