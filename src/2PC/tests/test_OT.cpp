...
void initGlobals(PackedZ2<N_SIZE>& b, const std::vector<uint64_t>& A);
...
void topelitz_Party1(const std::vector<uint64_t>& A,
                     PackedZ2<N_SIZE>& b, int index);

void topelitz_Party2(PackedZ2<N_SIZE>& y, PackedZ2<N_SIZE>& x, int index);
...
int main()
{
    ...
    preProc_OT(1); //preprocess for OT, generate Z
